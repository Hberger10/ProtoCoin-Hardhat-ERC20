...
describe("ProtoCoin Tests", function () {
  ...
  it("Should test", async function () {
    ...
    expect(true).to.equal(true); // Nota: A imagem mostra 'true' sendo comparado com '[true]'
  });

  it("name  test", async function () {
    const { protoCoin, owner, otherAccount } = await loadFixture(deployFixture);
    const name =  await protoCoin.name()
    expect(name).to.equal("ProtoCoin"); // Nota: A imagem mostra 'true' sendo comparado com '[true]'
  });

  it("symbol  test", async function () {
    const { protoCoin, owner, otherAccount } = await loadFixture(deployFixture);
    const symbol =  await protoCoin.symbol();
    expect(symbol).to.equal("Prc"); // Nota: A imagem mostra 'true' sendo comparado com '[true]'
  });

  it("decimals  test", async function () {
    const { protoCoin, owner, otherAccount } = await loadFixture(deployFixture);
    const decimals =  await protoCoin.decimals()
    expect(decimals).to.equal(18); // Nota: A imagem mostra 'true' sendo comparado com '[true]'
  });

  it("totalsuply test", async function () {
    const { protoCoin, owner, otherAccount } = await loadFixture(deployFixture);
    const totalsuply =  await protoCoin.totalsuply()
    expect(totalsuply).to.equal(1000n *10n **18n); // Nota: A imagem mostra 'true' sendo comparado com '[true]'
  });

  it("Balance_of test", async function () {
    const { protoCoin, owner, otherAccount } = await loadFixture(deployFixture);
    const balance =  await protoCoin.balanceOf(owner.address)
    expect(balance).to.equal(1000n *10n **18n); // Nota: A imagem mostra 'true' sendo comparado com '[true]'
  });
  ...
  it("transfer test", async function () {
    ...
    // Chamando a função 'transferFrom' com os argumentos certos
...
it("Approval test", async function () {
    const { protoCoin, owner, otherAccount } = await loadFixture(deployFixture);
  ...
  await expect(instance.transferFrom(owner.address,otherAccount.address, 1n))
    .to.be.revertedWith("Insuficcient allowance");
});
